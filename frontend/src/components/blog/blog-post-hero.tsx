...
export function BlogPostHero({
  ...
  const images = hasImages
    ? imageUrls.map((src, i) => ({
        src,
        aspect: (i % 2 === 0 ? "vertical" : "horizontal") as "vertical" | "horizontal",
      }))
    : [
        ...
        >
          {images.map((image, index) => (
            <div
              key={index}
              className={cn(
                "shrink-0 overflow-hidden rounded-xl snap-center bg-gray-100",
                image.aspect === "vertical"
                  ? "w-[180px] md:w-[240px] aspect-[3/4]"
                  : "w-[280px] md:w-[380px] aspect-[4/3] md:aspect-[16/10]"
              )}
              style={"src" in image ? undefined : { backgroundColor: imageColor }}
            >
